...
<form name="forms"  action="/reg" method="post" class="login-cont">
    <ul>
        <li>
            <input type="tel" name="phone" id="phone" placeholder="请输入您的手机号" >
            <span id="click"><a href="javascript:code()" class="iconfont icon-biyan">点击获取验证码</a></span>
        </li>
        <li>
	<span id="box">
      <input type="password"  name="password" id="password" placeholder="请输入6位数密码">
    </span>
     </li>

        <li><input type="tel" name="code" id="code" placeholder="请输入验证码" ></li>
    </ul>

    <a href="javascript:;" onclick="reg()" class="button text-center block">确认注册</a>
    <a href="javascript:;" class="text-center block font-color">忘记密码？</a>
    <p class="text-center">已有账户？ <a href="javascript:;" class="font-color" >立即登录</a></p>
</form>
...
<script>
    function code() {
        ...
        if(!isNaN(phone) && phone.length == 11){
            $.get('/user/code/'+phone,function (data) {
                if(data.code == 200){
                    layer.open({
                        content: '验证码发送成功'
                        ,skin: 'msg'
